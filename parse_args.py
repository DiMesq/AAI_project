...
def parse_args():
    ...
    parser_train.add_argument('--training-kind', '-k', required=True, dest='training_kind', choices=['new', 'resume', 'resume_cnn'], help="Either 'new' or 'resume' or 'resume_cnn'")
    ...
    parser_train.add_argument('--run-id', dest='run_id', help='Run id of the model to load')
    parser_train.add_argument('--num-epochs', '-E', dest='num_epochs', default=150, help='Model name (for loading custom model or for transfer learning)')
    ...
    parser_eval.add_argument('--run-id', required=True, dest='run_id', help='Run id of the model to load')
